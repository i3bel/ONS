...
struct TMDbSearchClient: Sendable {
    ...
    init(
        ...
        fetchTVSeriesByID: @escaping @Sendable (Int, Language) async throws -> BasicInfo? = { _, _ in nil },
        fetchMovieByID: @escaping @Sendable (Int, Language) async throws -> BasicInfo? = { _, _ in nil }
    ) {
    ...
    static func live(fetcher: InfoFetcher = .init()) -> Self {
        Self(
            ...
            fetchTVSeriesByID: { id, language in
                let series = try await fetcher.tvSeries(id, language: language)
                let posterURL = try? await fetcher.tmdbClient.imagesConfiguration
                    .posterURL(for: series.posterPath, idealWidth: 200)
                return BasicInfo(
                    name: series.name,
                    nameTranslations: [:],
                    overview: series.overview,
            ...
            fetchMovieByID: { id, language in
                let movie = try await fetcher.movie(id, language: language)
                let posterURL = try? await fetcher.tmdbClient.imagesConfiguration
                    .posterURL(for: movie.posterPath, idealWidth: 200)
                return BasicInfo(
                    name: movie.title,
                    nameTranslations: [:],
                    overview: movie.overview,
...
class TMDbSearchService {
    ...
    enum BatchPromptInput {
        case nameQuery(String)
        case tmdbID(Int, type: AnimeType)
    }

    nonisolated static func parseBatchPromptInput(_ line: String) -> BatchPromptInput {
        let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.lowercased().hasPrefix("tv:"),
           let id = Int(trimmed.dropFirst(3).trimmingCharacters(in: .whitespaces)) {
            return .tmdbID(id, type: .series)
        }
        if trimmed.lowercased().hasPrefix("movie:"),
           let id = Int(trimmed.dropFirst(6).trimmingCharacters(in: .whitespaces)) {
            return .tmdbID(id, type: .movie)
        }
            ...
            let chunkResults = try await withThrowingTaskGroup(
                ...
                for (offset, prompt) in chunk.enumerated() {
                    ...
                    group.addTask { [client] in
                        let parsed = Self.parseBatchPromptInput(prompt)
                        let topSeries: BasicInfo?
                        let topMovie: BasicInfo?

                        switch parsed {
                        case .nameQuery(let query):
                            async let seriesResults = client.searchTVSeries(query, language)
                            async let movieResults = client.searchMovies(query, language)
                            topSeries = try await seriesResults.first
                            topMovie = try await movieResults.first
                        case .tmdbID(let id, type: .series):
                            topSeries = try await client.fetchTVSeriesByID(id, language)
                            topMovie = nil
                        case .tmdbID(let id, type: .movie):
                            topSeries = nil
                            topMovie = try await client.fetchMovieByID(id, language)
                        case .tmdbID:
                            topSeries = nil
                            topMovie = nil
                        }
